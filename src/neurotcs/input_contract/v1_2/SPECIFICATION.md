...
## v1.2.0 addendum (Layer 3 anchor fields)

Released alongside `cross_sheet/manifest_data_consistency@1.0.0` (LAYER_3_DESIGN.md §9.3). v1.2.0 adds two new OPTIONAL manifest fields and one branch in the L3-conformance allOf rule. All v1.0 and v1.1 submissions remain valid under the v1.2 validator.

### New manifest fields

1. **`declares_continuous_biomarkers`** (boolean, optional)

   When `true`, the submission MUST include a `biomarkers` sheet with a UCUM-conformant `unit` field on every numeric row. v1.1 already enforced UCUM allowlist at the validator level when a biomarkers sheet was present; v1.2 adds a manifest-level *declaration* that downstream Layer 3 invariants can cross-check.

   Cross-sheet enforcement: `cross_sheet/manifest_data_consistency@1.0.0` invariant `continuous_biomarkers_declared_then_units_required`.

2. **`data_files.attribution`** (path string, optional unless L3)

   Directory path containing one attribution JSON file per (patient_id, visit_id) tuple in `predictions`. Required when `conformance_level == "L3"`.

   Cross-sheet enforcement: `cross_sheet/manifest_data_consistency@1.0.0` invariant `L3_conformance_requires_complete_attribution`.

### Updated L3 branch

The `allOf` block where `conformance_level == "L3"` now requires `data_files` to include `attribution` in addition to `predictions` and `patients`.

### Backward compatibility

- v1.0.0 submissions: valid (no new fields required; no L3-attribution branch fires unless conformance_level == "L3" — and v1.0 submissions cannot declare L3 because v1.0 doesn't ship continuous biomarkers).
- v1.1.0 submissions: valid (new fields are optional at L1/L2; new L3 requirement is the only behavior change, and v1.1 submissions targeting L3 were already expected to ship attribution).
- v1.2.0 submissions: all three contract versions accepted by `validate_submission()`.
